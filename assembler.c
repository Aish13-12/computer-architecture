...
void r_inst1(char function[5],char funcode[7],FILE*fp)
{
	...
	printf("%s",rs);
	char rt[6];
	if(strcmp(funcode,"001001")==0)
	{
		sprintf(rt,"%05lld",convert(31));
		printf("%s",rt);
	}	
...
void i_inst(char function[5],char funcode[7],FILE*fp)
{
	...
	{
		sprintf(addr,"%016lld",convert((constant-pc)/4));
		printf("%s",addr);
	}
	char bin_str[33];
	...
	printf(" (");
	bintohex(bin_str);
	printf(", %s %s, %s, %d)\n",function,reg_in,reg_out,constant);
}	
...
int main()
{
	...
	while(fscanf(fp,"%s",function)==1)
	{
	    ...
	    if(strcmp(function,"jalr")==0)
	    r_inst1("jalr","001001",fp);
